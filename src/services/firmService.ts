...
class FirmService {
  ...
  private async syncSubTables(client: any, firm: LawFirm): Promise<{ [key: string]: number }> {
    ...
    try {
      ...
      if (Array.isArray(data.partners) && data.partners.length > 0) {
        ...
        const { error } = await client.from('partners').upsert(rows, { onConflict: 'id' });
        if (!error) stats.partners = rows.length;
      }
      ...
      if (Array.isArray(data.practiceAreas) && data.practiceAreas.length > 0) {
        ...
        const { error } = await client.from('practice_areas').upsert(rows, { onConflict: 'id' });
        if (!error) stats.practiceAreas = rows.length;
      }
      ...
      if (Array.isArray(data.blogPosts) && data.blogPosts.length > 0) {
        ...
        if (!error) stats.blogPosts = rows.length;
      }
      ...
      if (Array.isArray(data.testimonials) && data.testimonials.length > 0) {
        ...
        if (!error) stats.testimonials = rows.length;
      }
      ...
      if (Array.isArray(data.offices) && data.offices.length > 0) {
        ...
        const { error } = await client.from('office_locations').upsert(rows, { onConflict: 'id' });
        if (!error) stats.offices = rows.length;
      }
      ...
      if (Array.isArray(data.caseStudies) && data.caseStudies.length > 0) {
        ...
        const { error } = await client.from('case_studies').upsert(rows, { onConflict: 'id' });
        if (!error) stats.caseStudies = rows.length;
      }
      ...
      if (Array.isArray(data.messages) && data.messages.length > 0) {
        ...
        const { error } = await client.from('consultation_inquiries').upsert(rows, { onConflict: 'id' });
        if (!error) stats.messages = rows.length;
      }
  ...
  public async syncAllToSupabase(): Promise<{ success: boolean; message: string; count?: number }> {
    ...
    try {
      ...
      // Sync each firm individually so one firm's issue never stops the others
